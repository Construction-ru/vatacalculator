...
namespace vatacalculator
{
    class Program
    {
        ...
        private static double РассчитатьСтоимостьТеплоизоляции(double H, CalculationData d, bool toFile = false)
        {
            ...
            var k1 = Math.Pow(1.0 + d.КредитнаяСтавка/100.0, d.СрокКредита);
            var k2 = Math.Pow(1.0 + d.ДисконтированиеЭлектроэнергии/100.0, d.ВремяЭксплуатацииТеплоизоляции - d.СрокКредита);
            ...
            // d.ВремяЭксплуатацииТеплоизоляции / d.ВремяЭксплуатацииНесущихКонструкций
            ...
            sb.AppendLine("Стоимость несущих конструкций " + a1.ToString("C") + ", с процентами " + (a1 * k1 * k2).ToString("C"));
            sb.AppendLine("Проценты за время эксплуатации теплоизоляции (в разах; например, число 1,0 при ставке 0% годовых) " + percent.ToString("F3") + ", k1 = " + k1.ToString("F3") + ", k2 = " + k2.ToString("F3"));
